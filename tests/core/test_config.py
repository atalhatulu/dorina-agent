...
class TestSettings:
    def test_settings_nested_configs(self):
        ...
        s = Settings()
        # Model should be in "provider/model" format (may change over time)
        assert "/" in s.model.default, f"Model should be provider/model format, got: {s.model.default}"
        assert s.model.provider
