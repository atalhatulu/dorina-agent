...
class Soul:
    ...
    def system_prompt(self) -> str:
        ...
        lines.append("- MAX 5 TOOL CALLS per user query. After 5, respond with what you have.")
        lines.append("- For system info: use a SINGLE `inxi -Fz` or `neofetch`. No follow-up commands needed.")
        lines.append("- Each tool call costs tokens. Skip tools for things you know directly.")
        lines.append("- If output is incomplete, use pipes (grep, wc, tail) not extra commands.")
        lines.append("- After write_file/patch: DO NOT re-read the file. Check succeeded, move on.")
        lines.append("- read_file: only for looking at source code or configs. For paths, use search_files.")
        lines.append("")
